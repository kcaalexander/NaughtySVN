...
static void
nsvn__simple_activate_username (GtkWidget *widget,
                                GladeXML *user_data)
{
  GtkWidget *password;

  password = glade_xml_get_widget (user_data, "auth_password_ent");
  if (password)
    gtk_widget_grab_focus ( GTK_WIDGET (password));
}

static void
nsvn__simple_activate_password (GtkWidget *widget,
                                GladeXML *user_data)
{
  GtkWidget *window;

  window = glade_xml_get_widget (user_data, "auth_dialog");
  if (window)
                            ...
                            apr_pool_t *pool)
{
  ...
  if (!dlg_gui)
  {
    ...
    return svn_error_create ( SVN_ERR_CANCELLED, NULL, "Error loading GUI");
  }
  ...
  g_signal_connect (G_OBJECT (window), "destroy",
                    ...
                    dlg_gui);
  g_signal_connect (G_OBJECT(username), "activate",
                    G_CALLBACK (nsvn__simple_activate_username),
                    dlg_gui);
  g_signal_connect (G_OBJECT(password), "activate",
                    G_CALLBACK (nsvn__simple_activate_password),
                    dlg_gui);
...
static void
nsvn__username_activate_username (GtkWidget *widget,
                                GladeXML *user_data)
{
  GtkWidget *window;
                              ...
                              apr_pool_t *pool)
{
  ...
  g_signal_connect (G_OBJECT (window), "destroy",
                    ...
                    dlg_gui);
  g_signal_connect (G_OBJECT(username), "activate",
                    G_CALLBACK (nsvn__username_activate_username),
                    dlg_gui);
                                      ...
                                      apr_pool_t *pool)
{
  ...
  g_string_printf (tempmessage, _("Error validating server certificate for '%s'\n\n\n"), realm);
  
  if (failures & SVN_AUTH_SSL_UNKNOWNCA)
    g_string_append (tempmessage, _("The certificate is not issued by a trusted authority. Use the\nfingerprint to validate the certificate manually!\n\n"));
    
  if (failures & SVN_AUTH_SSL_CNMISMATCH)
    g_string_append (tempmessage, _("The certificate hostname does not match.\n\n"));
                                 ...
                                 GladeXML *dlg_gui)
{
  ...
  fc = gtk_file_chooser_dialog_new ("Open client certificate",
                                    GTK_WINDOW(window),
                                     ...
                                     apr_pool_t *pool)
{
  ...
  string = g_strdup_printf ("Need a client certificate for '%s'", realm);
                                        ...
                                        apr_pool_t *pool)
{
  ...
  string = g_strdup_printf ("Passphrase for the client certificate on '%s'", realm);
  gtk_label_set_text (GTK_LABEL(message), string);
  g_free (string);
